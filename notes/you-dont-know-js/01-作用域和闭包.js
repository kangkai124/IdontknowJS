 ...
 *
 * 区分函数声明和函数表达式： 看 function 关键字出现在声明中的位置（整个声明中的位置）。
 * 如果 function 是第一个词，就是函数声明，否则就是函数表达式。
 *
 ...
 * 函数声明会被提升，函数表达式不会被提升。
 *
 * 变量标识符 foo 被提升，但是执行 foo() 时 foo = undefined，因此对
 * undefined 进行函数调用而导致非法操作，抛出 TypeError 异常。
 */
foo();    // TypeError!!!
var foo = function () {
  //
